...
1. How is the client sending data to the server? What protocol?
  The client establishes a connection with server first, then sends data using a write() call. The
protocols being used are TCP and AF_INET

2. What data is the client sending to the server?
  The client is sending an array of strings: Hello, Apple, Car, Green, Dog



Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  run_acceptor thread is passed a pointer to a struct acceptor_args, this contains an atomic_bool run
that controls the while loop, a struct list_handle *list_handle which is used to pass data to our
client args, and a pthread_mutex_t *list_lock which is our mutex for thread safety.

2. How are received messages stored?
  First messages are put into a buffer. Then messages are copied from buffer and stored using a linked
list, list appends new messages at the end of the list

3. What does `main()` do with the received messages?
  Main counts the number of messages received and checks if it matches the total count. If count
doesnt match total count it exits, if count does match total count it prints each message, after it
frees all nodes in linked list.

4. How are threads used in this sample code?
  In the sample code, threads are used to accept clients.


  Sockets are made non-blocking by the set_non_blocking function. It takes in a file descriptor and
  calls fcntl() with F_GETFL on it to get the file descriptors flags. Next, it calls fcntl() again
  with F_SETFL and flags | O_NONBLOCK, to set the file descriptors flags to non-blocking.

  The sockets that are made non-blocking are the server socket, and each client socket.

  The server socket is made non-blocking so that our accept() call does not stop our loop and
  threads from running. The client socket is made non-blocking so that our read() call does not
  block execution if there is no data. This allows the client threads to check for messages without
  getting stuck.
