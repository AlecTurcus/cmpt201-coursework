...
int main() {
  ...
  FILE *key = fopen("public_key.pem", "r");
  if (!key) {
    handle_error("Error");
  }

  pubkey = PEM_read_PUBKEY(key, NULL, NULL, NULL);
  // Verify each message
  for (int i = 0; i < 3; i++) {
...
#define MAX_FILE_SIZE 512
  ...
  ssize_t msg_len = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  ssize_t sig_len = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  signature[0] = 'l';
  ...
  if (1 != EVP_DigestVerifyUpdate(mdctx, message, msg_len)) {
    handle_error("update");
  }

  if (1 == EVP_DigestVerifyFinal(mdctx, signature, sig_len)) {
