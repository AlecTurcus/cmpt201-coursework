...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  if (*result_count == 0) {
    output[0].doubled_value = input->doubled_value;
    output[0].line_numbers[output[0].count++] = input->line_number;
    (*result_count)++;
    return;
  }
  ...
  output[*result_count].line_numbers[output[*result_count].count++] = input->line_number;
  (*result_count)++;
}
