...
int find_best_fit(struct header *free_list_ptr, uint64_t size) {
  ...
  while (free_list_ptr) {
    if (free_list_ptr->size >= size) {
      if (best_fit_id == -1 || (free_list_ptr->size < current_size)) {
        best_fit_id = free_list_ptr->id;
        current_size = free_list_ptr->size;
      }
    }
    free_list_ptr = free_list_ptr->next;
  }
...
int find_worst_fit(struct header *free_list_ptr, uint64_t size) {
  ...
  while (free_list_ptr) {
    if (free_list_ptr->size > size) {
      if (worst_fit_id == -1 || (current_size < free_list_ptr->size)) {
 ...
 *  add next size to curr size
 *  update pointer
 *
 *
 * If prev block free
 *  remove from free list
 *  add prev size to curr size
 *  update pointer
 *
 */
